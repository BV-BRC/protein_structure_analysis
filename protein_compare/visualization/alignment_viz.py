...
class AlignmentVisualizer:
    ...
    def generate_pymol_script(
        ...
        aligned_pdb_path: Optional[str | Path] = None,
    ) -> str:
        ...
        Args:
            ...
            aligned_pdb_path: Optional path to save the pre-aligned structure 2.
                             If provided, the transformation is applied to the PDB
                             file directly rather than in PyMOL.

        Returns:
            PyMOL script as string.
        """
        # Determine paths for loading structures
        struct1_path = f"{struct1.name}.pdb"

        if aligned_pdb_path:
            # Save pre-transformed structure 2
            self.save_aligned_pdb(struct2, alignment, aligned_pdb_path, apply_transform=True)
            struct2_path = str(aligned_pdb_path)
            use_transform = False
        else:
            struct2_path = f"{struct2.name}.pdb"
            use_transform = True
        ...
        script_lines = [
            ...
            "",
            "# Load structures",
            f'cmd.load("{struct1_path}", "{struct1.name}")',
            f'cmd.load("{struct2_path}", "{struct2.name}")',
            "",
        ]

        if use_transform:
            # Apply transformation in PyMOL (legacy approach, may not work reliably)
            script_lines.extend([
                "# Apply transformation to align structure 2 to structure 1",
                "# Note: If alignment looks wrong, use aligned_pdb_path option instead",
            ])

            rot = alignment.rotation_matrix  # u from tmtools (3x3)
            trans = alignment.translation_vector  # t from tmtools (3,)

            # For PyMOL column-vector convention: R = u^T, T = -(t @ u)
            R = rot.T
            T = -(trans @ rot)

            script_lines.append("# Transform structure 2 to align with structure 1")
            script_lines.append(f'cmd.transform_selection("{struct2.name}", [\\')
            script_lines.append(f"    {R[0,0]}, {R[0,1]}, {R[0,2]}, {T[0]},")
            script_lines.append(f"    {R[1,0]}, {R[1,1]}, {R[1,2]}, {T[1]},")
            script_lines.append(f"    {R[2,0]}, {R[2,1]}, {R[2,2]}, {T[2]},")
            script_lines.append(f"    0, 0, 0, 1])")
            script_lines.append("")
        else:
            script_lines.extend([
                "# Structure 2 has been pre-aligned and saved to:",
                f"# {struct2_path}",
                "",
            ])
    ...
    def save_aligned_pdb(
        ...
        Args:
            structure: ProteinStructure to save (structure 2 from alignment).
        ...
        if apply_transform:
            ...
            # tmtools returns u, t such that: aligned = (coords - t) @ u
            for atom in structure.biopython_structure.get_atoms():
                coord = atom.get_coord()
                new_coord = (coord - alignment.translation_vector) @ alignment.rotation_matrix
